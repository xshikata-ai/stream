...
function time_ago_id($datetime) {
    if (empty($datetime)) { return "beberapa saat yang lalu"; }
    $time = strtotime($datetime); $now = time(); $diff = $now - $time;
    if ($diff < 60) { return "baru saja"; }
    $minute = 60; $hour = $minute * 60; $day = $hour * 24; $month = $day * 30; $year = $day * 365;
    if ($diff < $hour) { $val = floor($diff / $minute); return "$val menit yang lalu"; }
    if ($diff < $day) { $val = floor($diff / $hour); return "$val jam yang lalu"; }
    if ($diff < $month) { $val = floor($diff / $day); return "$val hari yang lalu"; }
    if ($diff < $year) { $val = floor($diff / $month); return "$val bulan yang lalu"; }
    $val = floor($diff / $year); return "$val tahun yang lalu";
}
...
<div class="container">
    ...
    <div class="server-buttons-wrapper">
        <?php foreach ($sorted_servers as $index => $server_url): ?>
            <?php
            $host = parse_url($server_url, PHP_URL_HOST); $label = 'Server ' . ($index + 1);
            ...
            if ($is_vh) $label = 'VH'; elseif ($is_sw) $label = 'SW'; elseif ($is_dd) $label = 'DD';
            ?>
    ...
    <div class="video-title-box">
        <h1 class="title-main"><?php echo htmlspecialchars($video['original_title'] ?? 'Judul Tidak Diketahui'); ?></h1>
    </div>
    ...
    <div class="related-video-wrapper">
        <section class="video-section"><div class="section-header"><h2 class="section-title"><i class="ph-fill ph-film-strip icon"></i><span>Video Terkait</span></h2><div class="header-connector-line"></div></div>
            <?php $relatedVideos = []; if (!empty($video['category_id'])) { $relatedVideos = getRelatedVideosByCategoryFromDB($video['category_id'], $video['id'], 8); } if (empty($relatedVideos)) { $relatedVideos = getRandomVideosFromDB(8, $video['id']); } if (!empty($relatedVideos)): ?>
            ...
            <?php else: ?><p class="message info" style="text-align: center; padding: 2rem;">Tidak ada video terkait yang ditemukan.</p><?php endif; ?>
        </section>
    </div>
</div>
<?php else: ?>
    <div class="message error container">Video tidak ditemukan atau slug tidak valid.</div>
<?php endif; ?>
